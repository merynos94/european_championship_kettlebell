...
from .models.results import (
    KBSquatResult, OneKettlebellPressResult, OverallResult, PistolSquatResult,
    SeeSawPressResult, SnatchResult, TGUResult, TwoKettlebellPressResult)

from .models.constants import (
    AVAILABLE_DISCIPLINES, SNATCH, TGU, SEE_SAW_PRESS, KB_SQUAT, PISTOL_SQUAT,
    ONE_KB_PRESS, TWO_KB_PRESS
)
...
def player_link_display(obj, app_name="live_results"):
    ...
    if isinstance(obj, Player): player_instance = obj
    elif player and isinstance(player, Player): player_instance = player
...
class CategoryAdminForm(forms.ModelForm):
    disciplines = forms.MultipleChoiceField(
        ...
        required=False, label=_("Dyscypliny"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        else: self.fields["disciplines"].initial = []
    def save(self, commit=True):
        instance = super().save(commit=False)
        if hasattr(instance, 'set_disciplines'): instance.set_disciplines(self.cleaned_data.get("disciplines", []))
        else: instance.disciplines = self.cleaned_data.get("disciplines", [])
...
class PlayerAdmin(ImportExportModelAdmin):
    ...
    DISCIPLINE_TO_FIELD_MAP = {
        SNATCH: "get_snatch_score_display", TGU: "get_tgu_bw_percentage_display",
        SEE_SAW_PRESS: "get_ssp_bw_percentage_display", KB_SQUAT: "get_kbs_bw_percentage_display",
        PISTOL_SQUAT: "get_pistol_bw_percentage_display", ONE_KB_PRESS: "get_okbp_bw_percentage_display",
    ...
    def get_allowed_disciplines(self, obj: Player | None) -> set:
        ...
        if obj and obj.pk and hasattr(obj, "categories"):
            for category in obj.categories.all():
                ...
                if disciplines_in_cat: allowed_disciplines.update(disciplines_in_cat)
    ...
    def get_fieldsets(self, request, obj: Player | None = None):
        ...
        fieldsets = [ (_("Dane Podstawowe"), {"fields": ("name", "surname", "weight", "club", "categories", "tiebreak")}), ]
        results_fields = []
        for discipline_code, field_name in self.DISCIPLINE_TO_FIELD_MAP.items():
            if discipline_code in allowed_disciplines: results_fields.append(field_name)
        if results_fields or True: results_fields.append("get_overall_score_display")
        if results_fields:
            fieldsets.append( (_("Wyniki Obliczone (Tylko do odczytu)"), {"classes": ("collapse",),"fields": tuple(results_fields),}) )
        return fieldsets
    ...
    def get_readonly_fields(self, request, obj: Player | None = None):
        ...
        readonly = []
        for discipline_code, field_name in self.DISCIPLINE_TO_FIELD_MAP.items():
            if discipline_code in allowed_disciplines: readonly.append(field_name)
    ...
    def get_snatch_score_display(self, obj: Player) -> str: res = getattr(obj, "snatch_result", None); score = getattr(res, "result", None); return f"{score:.1f}" if score is not None else "---"
    @admin.display(description=_("TGU (%BW)"))
    def get_tgu_bw_percentage_display(self, obj: Player) -> str: res = getattr(obj, "tgu_result", None); bw = getattr(res, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("SSP (%BW)"))
    def get_ssp_bw_percentage_display(self, obj: Player) -> str: res = getattr(obj, "see_saw_press_result", None); bw = getattr(res, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("KBS (%BW)"))
    def get_kbs_bw_percentage_display(self, obj: Player) -> str: res = getattr(obj, "kb_squat_result", None); bw = getattr(res, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("Pistol (%BW)"))
    def get_pistol_bw_percentage_display(self, obj: Player) -> str: res = getattr(obj, "pistol_squat_result", None); bw = getattr(res, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("OKBP (%BW)"))
    def get_okbp_bw_percentage_display(self, obj: Player) -> str: res = getattr(obj, "one_kettlebell_press_result", None); bw = getattr(res, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("TKBP (%BW)"))
    def get_tkbp_bw_percentage_display(self, obj: Player) -> str: res = getattr(obj, "two_kettlebell_press_result", None); bw = getattr(res, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("Wynik Ogólny"), ordering="overallresult__total_points")
    def get_overall_score_display(self, obj: Player) -> str: overall = getattr(obj, "overallresult", None); total = getattr(overall, "total_points", None); return f"{total:.1f}" if total is not None else "---"
    ...
    def save_model(self, request, obj: Player, form, change):
        ...
        if change and obj.pk:
             try: old_obj = Player.objects.get(pk=obj.pk); old_categories_pks = set(old_obj.categories.values_list('pk', flat=True))
        ...
        if newly_added_category_pks:
            ...
            try:
                ...
                if created_any: print(f"Stworzono nowe domyślne rekordy wyników dla gracza {obj.id}.")
                else: print(f"Nie stworzono nowych domyślnych rekordów (prawdopodobnie już istniały) dla gracza {obj.id}.")
            except Exception as e: print(f"BŁĄD podczas tworzenia domyślnych rekordów dla gracza {obj.id}: {e}"); traceback.print_exc(); self.message_user(request, f"Wystąpił błąd podczas tworzenia domyślnych rekordów wyników: {e}", level='ERROR')
        print(f"Gracz {obj.id}: Uruchamiam przeliczenie wyników ogólnych...")
        try: update_overall_results_for_player(obj); print(f"Zakończono przeliczanie wyników ogólnych dla gracza {obj.id}.")
        except Exception as e: print(f"BŁĄD podczas przeliczania wyników ogólnych dla gracza {obj.id}: {e}"); traceback.print_exc(); self.message_user(request, f"Wystąpił błąd podczas przeliczania wyników ogólnych: {e}", level='ERROR')
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    DISCIPLINE_RELATED_NAMES = {
        SNATCH: "snatch_result", TGU: "tgu_result", SEE_SAW_PRESS: "see_saw_press_result",
        KB_SQUAT: "kb_squat_result", PISTOL_SQUAT: "pistol_squat_result",
        ONE_KB_PRESS: "one_kettlebell_press_result", TWO_KB_PRESS: "two_kettlebell_press_result",
    }
    DISCIPLINE_EXPORT_CONFIG = {
         SNATCH: {'header': "Snatch (kg x reps / wynik)", 'attributes': ['kettlebell_weight', 'repetitions', 'result'], 'template_snippet': 'admin/live_results/category/export_snippets/snatch.html',},
         TGU: {'header': "TGU (max kg / %BW)", 'attributes': ['max_result', 'bw_percentage', 'position'], 'template_snippet': 'admin/live_results/category/export_snippets/single_attempt.html',},
         PISTOL_SQUAT: {'header': "Pistol (max kg / %BW)", 'attributes': ['max_result', 'bw_percentage', 'position'], 'template_snippet': 'admin/live_results/category/export_snippets/single_attempt.html',},
         ONE_KB_PRESS: {'header': "OH Press (max kg / %BW)", 'attributes': ['max_result', 'bw_percentage', 'position'], 'template_snippet': 'admin/live_results/category/export_snippets/single_attempt.html',},
         SEE_SAW_PRESS: {'header': "SeeSaw Press (max kg / %BW)", 'attributes': ['max_score', 'bw_percentage', 'position'], 'template_snippet': 'admin/live_results/category/export_snippets/double_attempt.html',},
         KB_SQUAT: {'header': "KB Squat (max kg / %BW)", 'attributes': ['max_score', 'bw_percentage', 'position'], 'template_snippet': 'admin/live_results/category/export_snippets/double_attempt.html',},
         TWO_KB_PRESS: {'header': "2KB Press (max kg / %BW)", 'attributes': ['max_score', 'bw_percentage', 'position'], 'template_snippet': 'admin/live_results/category/export_snippets/double_attempt.html',},
    }
    ...
    def get_disciplines_list_display(self, obj: Category) -> str:
        ...
        if isinstance(disciplines, list): return ", ".join(disciplines) if disciplines else "---"
        return "N/A"
    def save_model(self, request, obj: Category, form, change):
        super().save_model(request, obj, form, change); print(f"Zapisano kategorię '{obj.name}'. Uruchamiam przeliczenie wyników...")
        try: update_discipline_positions(obj); update_overall_results_for_category(obj); print(f"Przeliczenie wyników dla kategorii '{obj.name}' zakończone."); self.message_user(request, f"Wyniki dla kategorii '{obj.name}' zostały pomyślnie przeliczone.", level='INFO')
        except Exception as e: print(f"BŁĄD podczas przeliczania wyników dla kategorii '{obj.name}' po zapisie: {e}"); traceback.print_exc(); self.message_user(request, f"Wystąpił błąd podczas przeliczania wyników dla kategorii '{obj.name}': {e}", level='ERROR')
    ...
    def export_results_as_html(self, request, queryset):
        if queryset.count() != 1:
            self.message_user(request, _("Proszę wybrać dokładnie jedną kategorię do eksportu wyników."), level='WARNING')
            return
        ...
        overall_results = OverallResult.objects.filter(
        ...
        if not overall_results.exists():
            self.message_user(request, _("Brak wyników do wyeksportowania dla kategorii: %s") % category.name, level='INFO')
            return
        ...
        if db_field.name == "player" and self.discipline_code:
            try:
                allowed_category_pks = {
                    cat.pk for cat in Category.objects.all()
                    if self.discipline_code in cat.get_disciplines()
                }

                if allowed_category_pks:
                    kwargs["queryset"] = Player.objects.filter(
                        categories__pk__in=allowed_category_pks
                    ).distinct().order_by('surname', 'name')
                    print(f"Ograniczono queryset graczy dla {self.__class__.__name__} do {kwargs['queryset'].count()} rekordów.")
                else:
                    kwargs["queryset"] = Player.objects.none()
                    print(f"Brak kategorii dla dyscypliny {self.discipline_code}, queryset graczy pusty dla {self.__class__.__name__}.")
...
class BaseSingleResultAdmin(BaseResultAdminMixin, admin.ModelAdmin):
    list_display = ('get_player_name', 'result_1', 'result_2', 'result_3', 'get_max_result_display', 'get_bw_percentage_display', 'position', 'get_player_categories')
    list_display_links = ('get_player_name',); list_filter = ("player__categories", "position", ("player__weight", admin.EmptyFieldListFilter))
    search_fields = ("player__name", "player__surname", "player__club__name"); readonly_fields = ("position", "get_max_result_display", "get_bw_percentage_display", "get_player_categories")
    list_select_related = ("player", "player__club"); fields = ('player', 'result_1', 'result_2', 'result_3', 'position', 'get_max_result_display', 'get_bw_percentage_display', 'get_player_categories')
    autocomplete_fields = ('player',)
    @admin.display(description=_("Zawodnik"), ordering="player__surname")
    def get_player_name(self, obj): player = getattr(obj, "player", None); return str(player) if player else _("Brak Gracza")
    @admin.display(description=_("Max Wynik"))
    def get_max_result_display(self, obj) -> str: max_res = getattr(obj, "max_result", None); return f"{max_res:.1f}" if max_res is not None else "---"
    @admin.display(description=_("% Masy Ciała"))
    def get_bw_percentage_display(self, obj) -> str: bw = getattr(obj, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("Kategorie"))
...
class BaseDoubleResultAdmin(BaseResultAdminMixin, admin.ModelAdmin):
    list_display = ('get_player_name', 'get_attempt_1_score_display', 'get_attempt_2_score_display', 'get_attempt_3_score_display', 'get_max_score_display', 'get_bw_percentage_display', 'position', 'get_player_categories')
    list_display_links = ('get_player_name',); list_filter = ("player__categories", "position", ("player__weight", admin.EmptyFieldListFilter))
    search_fields = ("player__name", "player__surname", "player__club__name"); readonly_fields = ("position", "get_max_score_display", "get_attempt_1_score_display", "get_attempt_2_score_display", "get_attempt_3_score_display", "get_bw_percentage_display", "get_player_categories")
    list_select_related = ("player", "player__club"); fields = ('player', ('result_left_1', 'result_right_1'), ('result_left_2', 'result_right_2'), ('result_left_3', 'result_right_3'), 'position', 'get_max_score_display', 'get_bw_percentage_display', 'get_player_categories', 'get_attempt_1_score_display', 'get_attempt_2_score_display', 'get_attempt_3_score_display')
    autocomplete_fields = ('player',)
    @admin.display(description=_("Zawodnik"), ordering="player__surname")
    def get_player_name(self, obj): player = getattr(obj, "player", None); return str(player) if player else _("Brak Gracza")
    @admin.display(description=_("Wynik Próby 1"))
    def get_attempt_1_score_display(self, obj) -> str: score = obj.get_attempt_score(1); return f"{score:.1f}" if score is not None else "---"
    @admin.display(description=_("Wynik Próby 2"))
    def get_attempt_2_score_display(self, obj) -> str: score = obj.get_attempt_score(2); return f"{score:.1f}" if score is not None else "---"
    @admin.display(description=_("Wynik Próby 3"))
    def get_attempt_3_score_display(self, obj) -> str: score = obj.get_attempt_score(3); return f"{score:.1f}" if score is not None else "---"
    @admin.display(description=_("Max Wynik (Suma)"))
    def get_max_score_display(self, obj) -> str: max_s = getattr(obj, "max_score", None); return f"{max_s:.1f}" if max_s is not None else "---"
    @admin.display(description=_("% Masy Ciała"))
    def get_bw_percentage_display(self, obj) -> str: bw = getattr(obj, "bw_percentage", None); return f"{bw:.2f}%" if bw is not None else "---"
    @admin.display(description=_("Kategorie"))
    def get_player_categories(self, obj) -> str: return get_player_categories_display(obj)
...
class SnatchResultAdmin(BaseResultAdminMixin, admin.ModelAdmin):
    ...
    fields = ('player', 'kettlebell_weight', 'repetitions', 'position', 'get_snatch_score_admin', 'get_player_categories')
    list_select_related = ("player", "player__club")
    autocomplete_fields = ('player',)
    @admin.display(description=_("Zawodnik"), ordering="player__surname")
    def get_player_name(self, obj: SnatchResult): player = getattr(obj, "player", None); return str(player) if player else _("Brak Gracza")
    @admin.display(description=_("Kategorie"))
    ...
    def get_snatch_score_admin(self, obj: SnatchResult) -> str: score = getattr(obj, "result", None); return f"{score:.1f}" if score is not None else "---"
    def get_queryset(self, request):
        qs = super().get_queryset(request); qs = qs.annotate(calculated_snatch_score=Case(When(kettlebell_weight__gt=0, repetitions__gt=0, then=F('kettlebell_weight') * F('repetitions')), default=Value(0.0), output_field=FloatField())); return qs
...
class OverallResultAdmin(admin.ModelAdmin):
    list_display = (
        "player_link", "get_player_categories_display",
    ...
    readonly_fields = (
        "player_link", "get_player_categories_display", "snatch_points",
    ...
    # DODANO: Blokowanie dodawania, edycji i usuwania wyników ogólnych przez admina
    ...
    DISCIPLINE_POINTS_FIELDS = {
        SNATCH: "snatch_points", TGU: "tgu_points", SEE_SAW_PRESS: "see_saw_press_points",
        KB_SQUAT: "kb_squat_points", PISTOL_SQUAT: "pistol_squat_points",
    ...
    def export_overall_results_as_html(self, request, queryset):
        ...
        for result in results:
            table_rows.append({
                'result': result,
                'categories_str': ", ".join([c.name for c in result.player.categories.all()]) if result.player.categories.exists() else "---"
            })
