...
class PlayerAdmin(ImportExportModelAdmin):
    ...
    def get_snatch_score_display(self, obj: Player) -> Optional[str]:
        try:
...
class SnatchResultAdmin(admin.ModelAdmin):
    ...
    def player_link(self, obj: SnatchResult):
        ...
        link = reverse("admin:twoja_aplikacja_player_change", args=[obj.player.id]) # Zmień 'twoja_aplikacja'
...
class TGUResultAdmin(admin.ModelAdmin):
    ...
    def player_link(self, obj: TGUResult):
        ...
        from django.urls import reverse
        from django.utils.html import format_html
        link = reverse("admin:twoja_aplikacja_player_change", args=[obj.player.id])
        return format_html('<a href="{}">{}</a>', link, obj.player)
    ...
    def get_bw_percentage_display(self, obj: TGUResult) -> Optional[str]:
...
class PistolSquatResultAdmin(admin.ModelAdmin):
     ...
     @admin.display(description=_("%BW"))
     def get_bw_percentage_display(self, obj: PistolSquatResult) -> Optional[str]:
...
class SeeSawPressResultAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "get_max_score_display", "position", "get_player_categories",
    )
    list_filter = ("player__categories", "position")
    search_fields = ("player__name", "player__surname")
    readonly_fields = ("player_link", "position", "get_max_score_display",
                       "get_attempt_1_display", "get_attempt_2_display", "get_attempt_3_display")
    list_select_related = ('player',)
...
class OverallResultAdmin(admin.ModelAdmin):
    list_display = (
        "player_link",
        "get_player_categories",
        "snatch_points", "tgu_points", "see_saw_press_points",
        "kb_squat_points", "pistol_squat_points",
        "tiebreak_points",
        "total_points",
        "final_position",
    )
    ...
    readonly_fields = ( # Wszystkie pola są obliczane
        "player_link", "snatch_points", "tgu_points", "see_saw_press_points",
        "kb_squat_points", "pistol_squat_points", "tiebreak_points",
        "total_points", "final_position", "get_player_categories",
    )
    list_select_related = ('player',)
