...
from .results.pistol_squat import PistolSquatResult # Importujemy PistolSquatResult
...
def update_discipline_positions(category: "Category") -> None:
    ...
    TGU, One KB Press, Pistol Squat, KB Squat, Two KB Press are ranked based on score / body_weight.
    (Docstring zaktualizowany o Pistol Squat)
    """
    from .player import Player
    from .results.overall import OverallResult # Potrzebny do aktualizacji punktów
    ...
    discipline_models_map = {
        ...
        PISTOL_SQUAT: PistolSquatResult, # Poprawnie mapuje
    ...
    ordering_logic = {
        SNATCH: "-result",
        TGU: "-tgu_bw_ratio",
        PISTOL_SQUAT: "-pistol_bw_ratio", # Zmieniono na ranking wg %BW
        ONE_KB_PRESS: "-okbp_bw_ratio",
        SEE_SAW_PRESS: "-max_ssp_score", # Pozostaje wg sumy (chyba że zmienimy?)
    ...
    for discipline in disciplines:
        ...
        if discipline == TGU:
             results_qs = results_qs.annotate(
                 ...
                 tgu_bw_ratio=Case(
                     When(player__weight__isnull=False, player__weight__gt=0, max_tgu_result__gt=0, then=(F("max_tgu_result") / F("player__weight"))),
                     default=Value(0.0), output_field=FloatField(),
                 )
             )
        # ===== ZMIANA TUTAJ (dodano adnotację dla %BW) =====
        elif discipline == PISTOL_SQUAT:
            results_qs = results_qs.annotate(
                max_pistol_result=Greatest(F("result_1"), F("result_2"), F("result_3"), Value(0.0))
            ).annotate( # Dodano adnotację dla %BW
                pistol_bw_ratio=Case(
                    When(
                        player__weight__isnull=False,
                        player__weight__gt=0,
                        max_pistol_result__gt=0, # Wynik musi być dodatni
                        then=(F("max_pistol_result") / F("player__weight")),
                    ),
                    default=Value(0.0),
        ...
        # ==================================================
            ...
            results_qs = results_qs.annotate(
                ...
                okbp_bw_ratio=Case(
                    When(player__weight__isnull=False, player__weight__gt=0, max_okbp_result__gt=0, then=(F("max_okbp_result") / F("player__weight"))),
                    default=Value(0.0), output_field=FloatField(),
                )
            )
            ...
            results_qs = (
                results_qs.annotate(
                    ...
                    kbs_bw_ratio=Case(
                        When(player__weight__isnull=False, player__weight__gt=0, max_kbs_score__gt=0, then=(F("max_kbs_score") / F("player__weight"))),
                        default=Value(0.0), output_field=FloatField(),
                    )
                )
            )
            ...
            results_qs = (
                results_qs.annotate(
                    ...
                    tkbp_bw_ratio=Case(
                        When(player__weight__isnull=False, player__weight__gt=0, max_tkbp_score__gt=0, then=(F("max_tkbp_score") / F("player__weight"))),
                        default=Value(0.0), output_field=FloatField(),
                    )
                )
            )
        ...
        last_score = None
        rank_counter = 0
        epsilon = 1e-6 # Tolerancja dla float

        for result in ordered_results:
            rank_counter += 1
            score = 0.0 # Domyślny score to 0.0
            try:
                if discipline == SNATCH: score = result.result or 0.0
                elif discipline == TGU: score = result.tgu_bw_ratio or 0.0
                # ===== ZMIANA TUTAJ =====
                elif discipline == PISTOL_SQUAT: score = result.pistol_bw_ratio or 0.0 # Używamy ratio
                # ========================
                elif discipline == ONE_KB_PRESS: score = result.okbp_bw_ratio or 0.0
                elif discipline == SEE_SAW_PRESS: score = result.max_ssp_score or 0.0
                ...
                pass # Pozostaw score jako 0.0

            # Porównaj z poprzednim wynikiem
...
def update_overall_results_for_category(category: "Category") -> None:
    ...
    players_in_category = Player.objects.filter(categories=category).prefetch_related(
        "snatch_result", "tgu_result", "pistol_squat_result", "see_saw_press_result",
        "kb_squat_result", "one_kettlebell_press_result", "two_kettlebell_press_result",
    )
    disciplines = category.get_disciplines()
    overall_updates = []
    ...
    for player in players_in_category:
        ...
        # Reset points
        overall_result.snatch_points = None; overall_result.tgu_points = None
        overall_result.see_saw_press_points = None; overall_result.kb_squat_points = None
        overall_result.pistol_squat_points = None; overall_result.one_kb_press_points = None
        overall_result.two_kb_press_points = None; overall_result.tiebreak_points = -0.5 if player.tiebreak else 0.0
        ...
        overall_result.pistol_squat_points = get_points(PISTOL_SQUAT, "pistol_squat_result") # Poprawnie pobierze pozycję
    ...
    final_pos_updates = []
    current_final_pos = 0; last_total_points = None; rank_counter = 0
