    ...
    for discipline in disciplines:
        ...
        if discipline == TGU:
            ...
            results_qs = (
                ...
                .annotate(
                    kbs_bw_ratio=Case(
                        When(player__weight__gt=0, max_kbs_score__gt=0, then=F("max_kbs_score") / F("player__weight")),
                        default=Value(0.0),
                        output_field=FloatField(),
                    )
                )
            ...
            results_qs = (
                ...
                .annotate(
                    tkbp_bw_ratio=Case(
                        When(
                            player__weight__gt=0, max_tkbp_score__gt=0, then=F("max_tkbp_score") / F("player__weight")
                        ),
                        default=Value(0.0),
                        output_field=FloatField(),
                    )
        ...
        for result in ordered_results:
            ...
            try:  # Pobierz wynik/ratio
                if discipline == SNATCH:
                    score = result.result or 0.0  # Snatch używa 'result'
                elif discipline == TGU:
                    score = result.tgu_bw_ratio or 0.0
                elif discipline == PISTOL_SQUAT:
                    score = result.pistol_bw_ratio or 0.0
                elif discipline == ONE_KB_PRESS:
                    score = result.okbp_bw_ratio or 0.0
                elif discipline == SEE_SAW_PRESS:
                    score = result.ssp_bw_ratio or 0.0
                elif discipline == KB_SQUAT:
                    score = result.kbs_bw_ratio or 0.0
                elif discipline == TWO_KB_PRESS:
                    score = result.tkbp_bw_ratio or 0.0
        ...
        if updates:
            try:
                ...
                print(f"BŁĄD bulk_update dla {discipline}: {e_bulk}")
