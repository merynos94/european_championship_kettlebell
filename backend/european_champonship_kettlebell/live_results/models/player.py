...
if TYPE_CHECKING:
    # Importuj typy, aby uniknąć błędów type hinting i zależności kołowych
...
class Player(models.Model):
    ...
    # Poprawiona składnia dla ForeignKey (jeśli używasz starszego Pythona/Django, możesz potrzebować tylko "SportClub")
    ...
    categories = models.ManyToManyField(  # Usunięto ["Category"]
        "Category",  # Nazwa modelu jako pierwszy argument
    ...
    def update_related_results(self) -> None:
        ...
        try:
            # Dynamiczne importy wewnątrz metody
            ...
            traceback.print_exc()
            # Rozważ logowanie błędu zamiast print w środowisku produkcyjnym
        finally:
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        super().save(*args, **kwargs)  # Najpierw zapisz gracza

        # Wywołaj aktualizację tylko jeśli nie jesteśmy już w trakcie
        # i tylko jeśli obiekt już istnieje (ma pk) lub jest nowy
        # (może być potrzebne dostosowanie logiki, kiedy dokładnie aktualizować)
        if not getattr(self, "_updating_results", False):
            try:
                # Można dodać warunek, np. if not is_new or dane_zostały_zmienione:
    ...
    def calculate_snatch_score(self) -> float | None:
        """Calculates the snatch score (weight * reps). Returns None if inputs invalid."""
