...
class Player(models.Model):
    ...
    def update_related_results(self) -> None:
        ...
        try:
            ...
            print(
                f"!!!!!!!!! BŁĄD KRYTYCZNY Player {self.id}: Błąd importu w update_related_results: {e} !!!!!!!!!")
        ...
        try:
            ...
            snatch_result_obj, created_raw = SnatchResult.objects.update_or_create(player=self, defaults={
                "result": snatch_score})
            ...
            tgu_result_obj, created_raw = TGUResult.objects.update_or_create(
                player=self, defaults={"result_1": self.tgu_weight_1 or 0.0, "result_2": self.tgu_weight_2 or 0.0,
            ...
            print(
                f"DEBUG TGU: update_best_result zwróciło: {updated_tgu}, new_best_result={best_tgu_obj.best_result}")
            ...
            pistol_result_obj, created_raw = PistolSquatResult.objects.update_or_create(
                player=self, defaults={"result_1": self.pistol_squat_weight_1 or 0.0,
            ...
            ssp_result_obj, created_raw = SeeSawPressResult.objects.update_or_create(
                player=self, defaults={"result_left_1": self.see_saw_press_weight_left_1 or 0.0,
            ...
            print(
                f"DEBUG SSP: update_or_create raw: created={created_raw}, max_score={getattr(ssp_result_obj, 'max_score', 'N/A')}")  # Użyj getattr, bo max_score to property
            ...
            kbs_result_obj, created_raw = KBSquatResult.objects.update_or_create(
                player=self, defaults={"result_left_1": self.kb_squat_weight_left_1 or 0.0,
            ...
            print(
                f"DEBUG KBS: update_best_result zwróciło: {updated_kbs}, new_best_result={best_kbs_obj.best_result}")
            ...
            okbp_result_obj, created_raw = OneKettlebellPressResult.objects.update_or_create(
                player=self, defaults={"result_1": self.one_kb_press_weight_1 or 0.0,
                                       "result_2": self.one_kb_press_weight_2 or 0.0,
                                       "result_3": self.one_kb_press_weight_3 or 0.0}
            )
            print(
                f"DEBUG OKBP: update_or_create raw: created={created_raw}, max_result={okbp_result_obj.max_result}")
            ...
            best_okbp_obj, created_best = BestOneKettlebellPressResult.objects.get_or_create(
                player=self)  # Czy tu wystąpi błąd NameError?
            print(
            ...
            tkbp_result_obj, created_raw = TwoKettlebellPressResult.objects.update_or_create(
                player=self, defaults={"result_left_1": self.two_kb_press_weight_left_1 or 0.0,
    ...
    def save(self, *args, **kwargs) -> None:
        ...
        try:
            ...
            print(f"==== Player {self.id}: self.update_related_results() zakończone.")
             ...
             print(f"!!!!!!!!! Player {self.id}: BŁĄD podczas wywołania update_related_results() z save(): {e_update} !!!!!!!!!")
             import traceback
             traceback.print_exc()
